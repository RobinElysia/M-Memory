...
class BucketManagerImpl(BucketManager):
    ...
    **Medoid computation** is deterministic: the node whose average cosine
    ...
    def find_candidates(
        ...
        """Find top-k candidate buckets via cosine similarity."""
        active = self.get_active_buckets()
        if not active:
            return []

        # Build query vector per bucket's Medoid
    ...
    def split_bucket(self, bucket: Bucket) -> list[Bucket]:
        """Split a bucket based on sub-cluster detection (placeholder)."""
        # Full sub-cluster detection via KMeans or similar is deferred.
        # For now, return the original bucket unchanged.
        return [bucket]
